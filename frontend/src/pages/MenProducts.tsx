...
// Keep categories for the filter buttons
...
export default function MenProducts() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    // Filter for MEN or UNISEX
    let result = apiProducts.filter((p: any) => p.gender === 'men' || p.gender === 'unisex');
    ...
    // Sort Logic (Updated with Number() for safety)
    switch (sortBy) {
      case 'price-asc':
      ...
      case 'featured':
      default:
        // You can define custom logic for featured, for now default to newest
        result.sort((a: any, b: any) => (b.isNew ? 1 : 0) - (a.isNew ? 1 : 0));
        break;
    }
  ...
  return (
    <Layout>
      {/* Hero Banner */}
      <div className="bg-gradient-to-r from-primary/20 to-secondary py-16 md:py-24">
        <div className="container mx-auto px-4 text-center">
          <h1 className="font-display text-4xl md:text-6xl font-bold text-foreground mb-4">
            Men's Collection
          </h1>
          <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
            Premium footwear designed for the modern man. From athletic to formal.
          </p>
        </div>
      </div>
            ...
            <>
                ...
                {filteredProducts.length === 0 && (
                  <div className="text-center py-16">
                    <p className="text-muted-foreground">No men's products found in this category.</p>
